...
const ProfileInfo: FC<OwnProps & StateProps> = ({
  ...
  const photos = userObj?.photos || chat?.photos || MEMO_EMPTY_ARRAY;
    ...
    return (
      <div className={styles.topicContainer}>
        <TopicIcon
          ...
          noLoopLimit
        />
        <h3 className={styles.topicTitle} dir={lang.isRtl ? 'rtl' : undefined}>
          {isLoading ? <div className={styles.chatnameLoader} /> : renderText(topic!.title)}
        </h3>
  ...
  function renderPhoto(isActive?: boolean) {
    ...
    return (
      <ProfilePhoto
        url={userObj?.profilePhoto && userObj.profilePhoto}
  ...
  function renderStatus() {
    if (userObj) {
      return (
        <div className={buildClassName(styles.status, 'status', isUserOnline(userObj, userStatus) && 'online')}>
          <span className="user-status" dir="auto">{getUserStatus(lang, userObj, userStatus)}</span>
        </div>
      );
    }
